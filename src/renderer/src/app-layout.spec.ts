...
describe('compact application layout', () => {
  ...
  it('removes rich-text timer markers, keeps two timer rows visible, and layers dialogs above list sections', () => {
    expect(appSource).not.toContain('inlineTimerHtml')
    expect(appSource).not.toContain('note-inline-timer')
    expect(appSource).not.toContain('设为核心')
    expect(appSource).not.toContain('设为排序')
